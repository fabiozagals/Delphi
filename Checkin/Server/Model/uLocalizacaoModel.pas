...
var
  LocalizacaoDao : TLocalizacaoDao;
  Localizacao  : TLocalizacaoModel;
begin
  Localizacao := TJson.JsonToObject<TLocalizacaoModel>(JSONObject);
  try
    LocalizacaoDao := TLocalizacaoDao.Create;
    try
      LocalizacaoDao.Incluir(Localizacao);
    finally
      FreeAndNil(LocalizacaoDao);
    end;

    Result := TJson.ObjectToJsonObject(Localizacao);
  finally
